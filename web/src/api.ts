...
export const api = {
  ...
  // End-of-conversation debrief: content-bank words to review + `new_words` (rich tier,
  // Slice 3c) the learner used that aren't in the bank yet — offered for their own deck.
  speechVocabReview: (sessionId: string) =>
    req<{ candidates: VocabCandidate[]; new_words?: string[]; over_budget?: boolean }>(
      `/speech/session/${encodeURIComponent(sessionId)}/vocab-review`,
      { method: "POST" }
    ),
  // One-click: enrich a spoken word (Slice D) and add it to the personal deck (Slice E).
  personalAddFromWord: (word: string, source = "speaking") =>
    req<{ card: PersonalCard | null; added: boolean; over_budget?: boolean }>(
      "/vocab/personal/from-word",
      { method: "POST", body: JSON.stringify({ word, source }) }
    ),
