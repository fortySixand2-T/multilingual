...
| H2 | word_bank integrity | confirmed | Issue 402: environnement-b1-01.e4 prompt said "global warming" but answer omitted "climatique" -- fixed by aligning prompt to "warming is faster" |
...
| H8 | content-sync + API | refuted | load_content returns 6 units, 18 lessons, 72 vocab; all 7 level tests pass; /content/levels lists b1 |
...
| id | title | gate split | notes |
|----|-------|-----------|-------|
| 401 | match_pairs English side drops secondary translations | rejected | By-design brevity for match exercise UI; primary meaning always correct |
| 402 | word_bank prompt "global warming" missing "climatique" | validated -> done | Fixed: prompt changed to "warming is faster" to match the French answer |

**Verdict: 1 content bug found and fixed. B1 vertical slice is sound.**
