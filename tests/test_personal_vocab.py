...
class ProfileRouter:
    """Returns a per-profile JSON payload; records which profiles were called. Lets one
    client exercise enrich + forms + examples, which use different profiles. A profile's
    value may be a single dict (fixed) or a list of dicts returned in order across
    successive calls (last repeats) — handy for testing example history accumulation."""
...
# --- API: word forms (vocab forms slice) --------------------------------------


def _add_card(client, fr, en, pos="", gender=""):
    return client.post(
        "/vocab/personal", json={"fr": fr, "en": en, "pos": pos, "gender": gender}
    ).json()["card"]["card_key"]


def test_forms_generates_then_serves_from_cache():
    router = ProfileRouter({"vocab_forms": {"forms": [{"label": "présent", "fr": "je mange"}]}})
    client, _ = _client(uid=30, router=router)
    key = _add_card(client, "manger", "to eat", pos="verb")

    r1 = client.post("/vocab/personal/forms", json={"card_key": key})
    assert r1.status_code == 200
    assert r1.json()["forms"] == [{"label": "présent", "fr": "je mange"}]
    assert r1.json()["cached"] is False
    assert router.calls == ["vocab_forms"]

    # second request is served from the stored forms — no new model call
    r2 = client.post("/vocab/personal/forms", json={"card_key": key})
    assert r2.json()["forms"] == r1.json()["forms"] and r2.json()["cached"] is True
    assert router.calls == ["vocab_forms"]  # unchanged
...
def test_forms_over_budget_returns_gracefully():
    ...
    client, _ = _client(uid=31, router=router)
    key = _add_card(client, "livre", "book", pos="noun", gender="m")

    async def burn():
        async with _Session() as s:
            await add_usage(s, 31, "vocab", 999_999, 0, date.today())
            await s.commit()

    _run(burn())
    r = client.post("/vocab/personal/forms", json={"card_key": key})
    assert r.json() == {"forms": [], "over_budget": True}
    assert router.calls == []  # no model call once over budget


def test_forms_unknown_card_is_404():
    client, _ = _client(uid=32)
...
def test_examples_are_fresh_each_press_and_accumulate_history():
    ...
    client, _ = _client(uid=33, router=router)
    key = _add_card(client, "chat", "cat", pos="noun", gender="m")

    r1 = client.post("/vocab/personal/examples", json={"card_key": key})
    assert [e["fr"] for e in r1.json()["examples"]] == ["Le chat dort."]

    r2 = client.post("/vocab/personal/examples", json={"card_key": key})
    # newest first, history preserved — a genuinely new sentence each press
    assert [e["fr"] for e in r2.json()["examples"]] == ["Un chat noir.", "Le chat dort."]
    assert router.calls == ["vocab_examples", "vocab_examples"]  # uncached: called each time

    # history is persisted on the card and surfaces in the deck listing
    deck = client.get("/vocab/personal").json()["cards"]
    card = next(c for c in deck if c["card_key"] == key)
    assert [e["fr"] for e in card["examples"]] == ["Un chat noir.", "Le chat dort."]
...
def test_examples_over_budget_keeps_existing_history():
    ...
    client, _ = _client(uid=34, router=router)
    key = _add_card(client, "livre", "book", pos="noun", gender="m")
    client.post("/vocab/personal/examples", json={"card_key": key})  # one sentence in history

    async def burn():
        async with _Session() as s:
            await add_usage(s, 34, "vocab", 999_999, 0, date.today())
            await s.commit()

    _run(burn())
    r = client.post("/vocab/personal/examples", json={"card_key": key})
    body = r.json()
    assert body["over_budget"] is True
    assert [e["fr"] for e in body["examples"]] == ["Je lis un livre."]  # existing kept
