...
- [2026-09-10] Fixed: content/b1/lessons/verbes-b1-01.yaml — QA round 056 #800: exercise `verbes-b1-01.e8` no longer accepts bare `"arriver"` (means "to arrive") as a translation of "to manage to" (`arriver à`), which contradicted the lesson's own grammar point (verb + à/de + infinitive); content/b1/lessons/verbes-b1-02.yaml — #801: exercise `verbes-b1-02.e8` no longer accepts bare `"plaindre"` (means "to pity") for "to complain" (`se plaindre`), keeping the legitimate `"se plaindre de"` variant; content/b2/lessons/verbes-b2-01.yaml — #802: exercise `verbes-b2-01.e8` `accept` list trimmed to just the accent-free spelling variant `"beneficier de"`, dropping the bare `"bénéficier"`/`"beneficier"` entries that let the preposition — this lesson's whole teaching point — be omitted; qa/issues/800-verbes-b1-01-e8-arriver-accept-wrong-meaning.md, qa/issues/801-verbes-b1-02-e8-plaindre-accept-wrong-meaning.md, qa/issues/802-verbes-b2-01-e8-beneficier-accept-drops-teaching-preposition.md — marked done with Fix notes
- [2026-08-30] Created: scripts/ensure_up.linux.sh — `@reboot` cron helper that brings the Docker compose stack up on the self-hosted box (waits for dockerd, idempotent `up -d`); belt-and-suspenders on top of the existing `restart: unless-stopped` policy + boot-enabled dockerd
- [2026-08-20] Changed: app/speech/examiner.py — `canned_opener()` + `_CANNED_OPENERS` (per-mode canned greetings); app/speech/api.py — `POST /speech/opener` now serves a canned, unbilled greeting for free conversation (no topic) — no LLM call — while topic openers still go through the LLM; the canned audio is synthesized once per mode and shared across sessions via a `speech/canned-{mode}.wav` storage key (`_canned_audio_key`); tests/test_speech.py — canned-opener tests (no LLM, cached audio synthesized once, empty-transcript turn) and topic-opener test now seeds a topic
- [2026-08-20] Added: app/speech/examiner.py — `SpeakingExaminer.opener()` so the examiner speaks first on a fresh session (LLM greeting, no STT), billed against the same daily ledger; app/speech/api.py — new `POST /speech/opener` (persists an empty-transcript opener turn, lazy TTS) and `_recent_history` now skips the blank opener utterance so it doesn't leak in as a user message; web/src/api.ts — `postSpeechOpener`; web/src/screens/Speaking.tsx — auto-fire the opener on an empty session (mount/topic/level), render opener as an examiner-only card, best-effort autoplay; and fix so a genuine level change resets the conversation (new session) instead of leaving/appending to the previous level's transcript
- [2026-08-20] Modified: tests/test_speech.py, web/src/screens/Speaking.test.tsx — opener tests (persists empty-transcript turn, start-directive prompt, greeting fed back without blank user msg, over-budget skips LLM, 503 when disabled; frontend: opens on empty session, suppressed on restored history, level change starts a fresh opener)
